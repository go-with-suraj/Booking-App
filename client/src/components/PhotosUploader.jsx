...
export const PhotosUploader = ({ addedPhotos, onChange }) => {
  ...
  async function addPhotoByLink(e) {
    e.preventDefault();
    try {
      const { data: filename } = await axios.post("/api/uploads/by-link", {
        link: photoLink,
      });
      onChange((prev) => {
  ...
  async function uploadPhoto(e) {
    ...
    const { data: filenames } = await axios.post("/api/uploads", data, {
          ...
          addedPhotos.map((link) => (
            <div key={link} className="h-32 flex relative">
              ...
              <button
                ...
                className=" cursor-pointer absolute bottom-1 right-1 text-white bg-black/50 rounded-2xl py-2 px-3"
              >
                <svg
              ...
              <button
                ...
                className=" cursor-pointer absolute bottom-1 left-1 text-white bg-black/50 rounded-2xl py-2 px-3"
              >
                {link === addedPhotos[0] && (
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
